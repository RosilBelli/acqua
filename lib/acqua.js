...
Acqua.prototype.importModule = function (modulePath, reload, watch) {
    var result,
        $module = modulePath,
        name;
    ...
    if (_.isFunction($module)) {
        ...
        if (name === null) {
            ...
            if (this.hotswap && watch) {
                fs.watchFile(modulePath, { persistent : true, interval : 300 }, () => {
                    this.fileChanged(modulePath, result);
                });
                result.name = name;
            }
        }
    ...
    if (file.indexOf('.js') < 0) {
        ...
        if (importFunction !== undefined && _.isFunction(importFunction)) {
            ...
            try {
                ...
                if (this.hotswap) {
                    fs.watchFile(location, { persistent : true, interval : 300 }, () => {
